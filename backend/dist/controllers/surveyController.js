...
exports.submitResponse = exports.getSurvey = void 0;
...
const submitResponse = async (req, res) => {
    try {
        ...
        const missingRequiredQuestions = requiredQuestions.filter((q) => !answeredQuestionIds.includes(q.id));
        if (missingRequiredQuestions.length > 0) {
            return res.status(400).json({
                message: 'Missing answers for required questions',
                missingQuestions: missingRequiredQuestions.map((q) => q.id)
            });
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            // Create all answers
            const answerPromises = answers.map(answer => tx.answer.create({
                data: {
